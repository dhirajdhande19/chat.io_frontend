...
const findUser = (e) => {
  ...
  function searchUser(user){
    return String(user.name).toLowerCase() == String(searchName).toLowerCase();
  }
      ...
      {/* Page Header */}
        <form className=" search flex mt-3 gap-3 col-span-4 col-start-2 sm:">
           <input onKeyDown={(e) => {
            ...
            <div>
              <button onClick={findUser}  className="btn sm:h-12 bg-orange-500">Search</button>
            </div>
            {/* {console.log(searchName)} */}
            {isLoggedin ? <button onClick={()=>{
              clearToken();
              return navigate("/");
            }} className="btn sm:h-12 bg-orange-500">LogOut</button> : <button onClick={() => {
              return navigate('/')
            }} className="btn sm:h-12 bg-orange-500">SignUp</button> }
        </form>
        

      <div className="mb-8 ">
        <h1 className="sm:text-4xl mt-4 text-lg font-bold text-gray-900">All Users</h1>
